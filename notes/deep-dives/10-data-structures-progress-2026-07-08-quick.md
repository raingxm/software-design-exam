...
> 完整进度地图见 [长版进度 §0](../deep-dives/09-data-structures-progress-2026-07-08.md#0-第三章整体地图-vs-进度)

- **循环队列**：`rear` 指下一个空位；空 `front==rear`；满 `(rear+1)%m==front`；个数 `(rear-front+m)%m`；最多 `m-1`
- **树还原**：能唯一的是「中序+前序/后序」；**前序+后序不唯一**（缺左右分界）
- **快排第一趟**：挖坑填数 — 右找小填坑、左找大填坑、pivot 归位（2026-07-08 已理解）
- **冒泡第一趟**：相邻比、大的往后换 → 最大值沉底；稳定 O(n²)（2026-07-08 已理解）
- **四排序识别**：冒泡沉底 / 选择放头 / 插入前缀 / 快排 pivot 居中
- **稳定性**：「冒插归基」稳定，「选快堆希」不稳定
- **堆排序**：建堆一次 → 每轮换顶沉底 → 只下沉 index 0（不重新建堆）→ 见 `11-heapsort-tutorial.md`
- **哈夫曼**：每轮取最小两个合并，共 `n-1` 轮；WPL 合并和或逐叶子；左0右1读编码（2026-07-08 已理解）
