...
> **深挖对比**：[V 模型 · 集成策略（桩/驱动）](../deep-dives/18-v-model-integration-tutorial.md)
...
> **深挖**：[黑盒边界 · 白盒覆盖](../deep-dives/17-software-testing-tutorial.md)（2026-07-22）· [V 模型 · 集成桩/驱动](../deep-dives/18-v-model-integration-tutorial.md)（课后补充）
...
不要把覆盖标准硬背成单一强弱链：**条件覆盖与判定覆盖互不包含**；判定/条件覆盖要求两者同时满足。在语句、判定、条件、路径四个常见选项中，路径覆盖程度最高。

注意陷阱：

- **条件覆盖**与**判定覆盖**互不包含（两个方向都能构造反例）。
- **路径覆盖**最强但路径爆炸，实际难全做。
- McCabe **环复杂度** \(V(G)=e-n+2\)（或判定数+1）→ 独立路径条数下限，和白盒路径相关。

### 5. 集成策略

> **深挖对比**：[桩 vs 驱动 · 与 V 模型对照](../deep-dives/18-v-model-integration-tutorial.md)
...
- 离散“第几天”题：若题目规定活动最早从第 1 天开始，则网络图中顶点最早时间 `t` 表示前 `t` 天已经结束，紧后活动从第 `t+1` 天开始。**问总工期不加 1，问活动从第几天开始才加 1**。例如 AB 持续 3 天并从第 1 天开始，占第 1～3 天，BD 从第 4 天开始。
...
连续式能力等级易错：题干只说“把可标识输入工作产品转换为可标识输出工作产品、支持过程域特定目标” → **CL1 已执行**；出现计划、跟踪、资源、责任、配置管理等管理纪律，才偏 **CL2 已管理**。
...
- [ ] 开场：「开始第 10 章网络，子网划分」或「计组 30 题错题回炉」
- [ ] 睡前 30 秒：白盒强度顺序 +「顶向下→桩」
- [ ] 第 5 章已学完；勿同时新开第 6/7 章长笔记
- [ ] 入口见 [交接看板](../deep-dives/01-learning-logs.md)
